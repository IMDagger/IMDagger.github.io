...
(c) https://github.com/getpelican/pelican-plugins/tree/master/twitter_bootstrap_rst_directives
...
class CleanHTMLTranslator(PelicanHTMLTranslator):
    ...
    def visit_reference(self, node):
        PelicanHTMLTranslator.visit_reference(self, node)
        ref_raw = self.body[-1]
        if 'data-attributes' in node:
            parts = ' '.join('data-{0}="{1}"'.format(att_name, self.attval(unicode(v))) \
                             for att_name, v in node['data-attributes'].iteritems())
            self.body[-1] = ref_raw[:-1] + parts + ref_raw[-1]
    ...
    def visit_table__(self, node):
        self.context.append(self.compact_p)
        self.compact_p = True
        classes = ' '.join(['docutils', self.settings.table_style]).strip()
        # without special border
...
def del_role(name, rawtext, text, lineno, inliner,
    ...
    """
        This function creates an inline console input block as defined in the twitter bootstrap documentation
        overrides the default behaviour of the del role
    ...
    """
    new_element = nodes.literal(rawtext, text)
    new_element.set_class('del')

    return [new_element], []

def underline_role(name, rawtext, text, lineno, inliner,
             options={}, content=[]):
    """
        This function creates an inline console input block as defined in the twitter bootstrap documentation
        overrides the default behaviour of the u role
    ...
    """
    new_element = nodes.literal(rawtext, text)
    new_element.set_class('underline')
...
def register_roles():
    ...
    rst.roles.register_local_role('underline', underline_role)
